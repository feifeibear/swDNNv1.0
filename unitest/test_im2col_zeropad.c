...
#define Type float
  ...
  gettimeofday(&t1, NULL);
  for(i = 0; i < batch_size; ++i)
    swim2col_zeropad_f(data_im + i*im_size,channels,height,width,kernel_h,kernel_w,
                pad_h,pad_w,stride_h,stride_w,dilation_h,dilation_w,zero_pad_data_col + i*pad_col_size);
  gettimeofday(&t2, NULL);
