 ...
 ************************************************************************/
#ifndef CONVLAYER_IMPL_V2_
#define CONVLAYER_IMPL_V2_
#include <stdio.h>
            ...
            for(cKc=0; cKc < K; ++cKc){
              for(cRi=0; cRi < Ri+2*pad; cRi++){
                for(cCi=0; cCi < Ci+2*pad; cCi++){
                  int cRi_real = cRi - pad;
                  int cCi_real = cCi - pad;
                  int cRo_real = cRi + cKr - (K-1);
                  int cCo_real = cCi + cKc - (K-1);
                  if( !(cCi_real >= 0 && cCi_real < Ci &&
                        cRi_real >= 0 && cRi_real < Ri) )
                  continue;
                  if( !(cCo_real >= 0 && cCo_real < Co &&
                        cRo_real >= 0 && cRo_real < Ro) )
                  continue;
                  *(in_grad+inGetIdx(cB, cNi, cRi_real, cCi_real, B, Ni, Ri, Ci)) +=
                  *(out_grad+outGetIdx(cB, cNo, cRo_real, cCo_real, B, No, Ro, Co)) *
                  *(weight+weightGetIdx( cNo, cNi,  K-1-cKr, K-1-cKc, No, Ni, K));
                }//cCo
              }//cRo 
            }//cKc
