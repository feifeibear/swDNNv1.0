...
//B, Ni, Ci, Ri
  SIMDType* local_input  = (SIMDType*) (long)ldm_malloc(sizeof(Type)*Ni*B/8/8*2);
  int local_input_size = Ni*B/8/8/SIMDSIZE;
//No, Ni, K, K
  Type* local_weight = (Type*) (long)ldm_malloc(sizeof(Type)*Ni*No/8/8*2);
...
//1st weight_load
  ...
  for(CoStart=0; CoStart<Co; CoStart+=CStride){
    ...
    for(cRo=0; cRo<Ro; ++cRo){
      ...
      dgemmasm((Type*)(local_input  + input_calc_index*local_input_size),
